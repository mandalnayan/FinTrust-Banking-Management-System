	...
	public void approveCardRequests(CardRequest cr) {
      System.out.println(cr);
        ...
        String sql = "INSERT INTO cards " +
                     "(user_id, card_number_masked, last4, expiry_date, provider, card_type, account_number) " +
                     "VALUES (?,?,?,?,?,?,?)";
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            pst.setDate(4, java.sql.Date.valueOf(LocalDate.now().plusYears(3)));
            if(cr.getCard_category().equalsIgnoreCase("visa"))
            	 pst.setString(5, "visa"); 
            else {
            	
            	if(cr.getCard_category().equalsIgnoreCase("mastercard"))
            		 pst.setString(5, "mastercard");
            	else
            		 pst.setString(5, "rupay"); 
            ...
            else
            {
            	if(cr.getCard_category().equalsIgnoreCase("Debit Card")) {
            	   pst.setString(6, "Debit");
            		...
            		pst.setString(6, "Prepaid");
            	}
            	}  
            
            pst.setLong(7, cr.getAccount_no());
